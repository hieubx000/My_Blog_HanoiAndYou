...
const getAllCategory = async(req, res) => {
    ...
    // FILTER
    const keywordFilter = keyword ? { title: { $regex: new RegExp(keyword, 'i') } } : {};
...
const createCategory = async(req, res) => {
    ...
    const existedCategory = await CategoryModel.findOne({ title })
    if (existedCategory) {
...
const updateCategory = async(req, res) => {
    ...
    const updateCategory = await CategoryModel.findOneAndUpdate({ slug }, { title }, { new: true })
